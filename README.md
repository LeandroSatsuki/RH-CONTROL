...
A produção utiliza o cliente `1.0.10` com o servidor `1.0.8`, em uma arquitetura cliente-servidor:
...
- `Nexo-Cliente-Setup-1.0.10.exe` para instalação manual do cliente.
...
4. Execute também `Nexo-Cliente-Setup-1.0.10.exe` para usar o Nexo no computador principal.
...
1. Instale somente `Nexo-Cliente-Setup-1.0.10.exe`.
...
cd ..
.\scripts\build-server-package.ps1 -Version 1.0.8
.\scripts\build-separated-installers.ps1 -Version 1.0.8
```

Antes da entrega, valide hashes e tamanhos:

```powershell
Get-FileHash .\entregas\Nexo-Servidor-Setup-1.0.8.exe -Algorithm SHA256
Get-FileHash .\entregas\Nexo-Servidor-Atualizador-1.0.8.exe -Algorithm SHA256
Get-FileHash .\entregas\Nexo-Cliente-Setup-1.0.8.exe -Algorithm SHA256
```
