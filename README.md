...
A versão `1.0.6` utiliza uma arquitetura cliente-servidor:
...
- lançamentos mensais de MEI, cesta básica e premiação, com elegibilidade, rascunho recuperável, confirmação e integração ao Custo/Folha;
...
- `Nexo-Servidor-Setup-1.0.6.exe` para primeira instalação;
- `Nexo-Servidor-Atualizador-1.0.6.exe` para servidor já instalado;
- `Nexo-Cliente-Setup-1.0.6.exe` para instalação manual do cliente.

### Computador principal

1. Execute `Nexo-Servidor-Setup-1.0.6.exe` como Administrador na primeira instalação.
...
4. Execute também `Nexo-Cliente-Setup-1.0.6.exe` para usar o Nexo no computador principal.
...
1. Instale somente `Nexo-Cliente-Setup-1.0.6.exe`.
...
Em um servidor já instalado, execute `Nexo-Servidor-Atualizador-1.0.6.exe` como Administrador. Esse entregável não faz instalação inicial: ele exige `C:\Nexo\.env`, cria e valida o backup, atualiza os arquivos, aplica migrations e só conclui depois que a API responder pela tarefa automática. Use `Nexo-Servidor-Setup-1.0.6.exe` apenas para uma máquina sem servidor ou para reparo assistido.
...
cd ..
.\scripts\build-server-package.ps1 -Version 1.0.6
.\scripts\build-separated-installers.ps1 -Version 1.0.6
```

Antes da entrega, valide hashes e tamanhos:

```powershell
Get-FileHash .\entregas\Nexo-Servidor-Setup-1.0.6.exe -Algorithm SHA256
Get-FileHash .\entregas\Nexo-Servidor-Atualizador-1.0.6.exe -Algorithm SHA256
Get-FileHash .\entregas\Nexo-Cliente-Setup-1.0.6.exe -Algorithm SHA256
```
