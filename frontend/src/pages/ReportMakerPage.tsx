...
import { DemoBenefitDistribution, DemoEmployee, DemoMovement } from "../mocks/demoTypes";
  ...
  return (
    <div className="report-maker-page">
      ...
      <section className="panel maker-commandbar">
        ...
        <label>Competência<select value={competency} onChange={event => setCompetency(event.target.value)}>{demoCompetencies.map(item => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label>
        ...
        ><option value="">Novo relatório</option>{templates.map(template => <option key={template.id} value={template.id}>{template.name}</option>)}</select></label>
...
function sanitizeTemplate(value: unknown): ReportTemplate | null {
  ...
  const fields = Array.isArray(raw.fields)
    ? raw.fields.flatMap(field => {
      ...
      const meta = fieldLibrary.find(item => item.id === candidate.id);
      const requested = aggregators.includes(candidate.aggregator as Aggregator) ? candidate.aggregator as Aggregator : defaultAggregator(meta);
      const allowed = aggregationOptions(meta).some(option => option.value === requested);
      return [{ id: candidate.id, aggregator: allowed ? requested : defaultAggregator(meta) }];
    })
    : [];
