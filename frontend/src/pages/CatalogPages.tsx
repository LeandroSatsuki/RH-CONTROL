...
export function CentersPage({ token, user, embedded = false }: { token: string; user: User; embedded?: boolean }) {
  ...
  const catalogCompanyId = selectedCompany.id || companies.find(item => item.is_primary)?.id || companies.find(item => item.id !== 0)?.id || 1;
  ...
  const load = async () => {
    ...
    setError("");
    try {
      setItems(await api<ResultCenter[]>(`/result-centers?company_id=${catalogCompanyId}`, {}, token));
    } catch (err) {
  ...
  useEffect(() => { void load(); }, [token, catalogCompanyId]);
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await api(editingId ? `/result-centers/${editingId}?company_id=${catalogCompanyId}` : `/result-centers?company_id=${catalogCompanyId}`, {
        method: editingId ? "PATCH" : "POST",
        body: JSON.stringify({ code, name, color: draft.color, ...(editingId ? {} : { company_id: catalogCompanyId, active: true }) })
      }, token);
  ...
  async function toggle(item: ResultCenter) {
    try {
      await api(`/result-centers/${item.id}?company_id=${catalogCompanyId}`, { method: "PATCH", body: JSON.stringify({ active: !item.active }) }, token);
      await load();
...
export function TypesPage({ token, user, embedded = false }: { token: string; user: User; embedded?: boolean }) {
  ...
  const load = async () => {
    ...
    setError("");
    try {
      setItems(await api<EmploymentType[]>(`/employment-types?company_id=${catalogCompanyId}`, {}, token));
    } catch (err) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await api(editingId ? `/employment-types/${editingId}?company_id=${catalogCompanyId}` : `/employment-types?company_id=${catalogCompanyId}`, {
        method: editingId ? "PATCH" : "POST",
        body: JSON.stringify({ name, has_charges: draft.hasCharges, ...(editingId ? {} : { company_id: catalogCompanyId, active: true }) })
      }, token);
  ...
  async function toggle(item: EmploymentType) {
    try {
      await api(`/employment-types/${item.id}?company_id=${catalogCompanyId}`, { method: "PATCH", body: JSON.stringify({ active: !item.active }) }, token);
      await load();
