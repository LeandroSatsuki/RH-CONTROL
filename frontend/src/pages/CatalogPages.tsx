...
function Catalog({ title, subtitle, error, form, children }: { title: string; subtitle: string; error: string; form: React.ReactNode; children: React.ReactNode }) {
  return <><div className="page-title"><div><span className="eyebrow">Cadastros</span><h1>{title}</h1><p>{subtitle}</p></div></div><ErrorMessage message={error} />{form && <div className="panel">{form}</div>}<div className="panel list">{children}</div></>;
}
