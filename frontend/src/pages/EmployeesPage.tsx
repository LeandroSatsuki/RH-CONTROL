...
export function EmployeesPage({ token, user }: { token: string; user: User }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setOpen(false);
      setSuccess("Colaborador criado em modo demonstração.");
      void load();
  ...
  return <>
    <div className="page-title">
      ...
      <div className="actions">
        {user.role === "ADMIN" && <button className="primary" onClick={() => setOpen(!open)}>{open ? "Cancelar" : "Novo colaborador"}</button>}
    ...
    {selected && <EmployeeDrawer employee={selected} user={user} onClose={() => setSelected(null)} onAction={simulate} />}
  </>;
}

function EmployeeDrawer({ employee, user, onClose, onAction }: { employee: DemoEmployee; user: User; onClose: () => void; onAction: (message: string, adminOnly?: boolean) => void }) {
  ...
  return <div className="drawer-backdrop" onClick={onClose}><aside className="drawer wide" onClick={event => event.stopPropagation()}>
    ...
    <div className="drawer-actions">
      {user.role === "ADMIN" ? <>
        ...
        <button className="secondary" onClick={() => onAction("Salário adicionado ao histórico em modo demo.", true)}>Adicionar salário</button>
    ...
    <Section title="Histórico salarial" items={employee.salary_history.map(item => `${date(item.date)} - ${money.format(item.amount)} (${item.reason})`)} />
    <Section title="Histórico de movimentações" items={employee.movement_history.map(item => `${date(item.date)} - ${item.description}`)} />
