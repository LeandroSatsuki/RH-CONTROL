...
def create_result_center(payload: ResultCenterCreate, db: DbSession, _: AdminUser) -> ResultCenter:
    if db.scalar(select(ResultCenter).where(ResultCenter.code == payload.code)):
        raise HTTPException(status_code=409, detail="Código de Centro de Resultado já existe")
